...
printf '{"hookSpecificOutput":{"hookEventName":"SessionStart","additionalContext":"No ADR found for branch \"%s\" (.context/decisions/branches/%s/%s.md). Offer to create one using the ADR system (.context/decisions/ + .context/decisions/adr-index.toml)."}}\n' \
  "$BRANCH" "$TYPE" "$SLUG"
